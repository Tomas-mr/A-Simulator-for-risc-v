...
#define CSW	(CFUNCT3==0b110)&&(COPCODE==0b00)
#define CFLD ((COPCODE == 0b00) && (CFUNCT3 == 0b001))
...
{
	unsigned int rd_ = (insn_16 >> 7) & 0b111;
	unsigned int uimm = (cimm_12 << 5) + (cimm_6_2);
	reg32[8 + rd_] = ((unsigned int)reg32[8 + rd_]) >> uimm;
	return pc + 2;
}
...
{
	unsigned int rd_ = (insn_16 >> 7) & 0b111;
	unsigned int rs2_ = (insn_16 >> 2) & 0b111;
	reg32[8 + rd_] = reg32[8 + rd_] & reg32[8 + rs2_];
	return pc + 2;
}
