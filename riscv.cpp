...
void Execuction(void) {
    for (;;cnt++) {
        ...
        reg32[0] = 0;
        COPCODE = x & (0b11);
        fprintf(outfile, "pc: %#4X\t    读取的字节：%#04X", pc, x);
        ...
        if (pc_add == 4)
        {
            ...
            CDecoder();
            instr_flow[cnt] =(unsigned int)( insn_16 & 0xffff);
            if (CFSD)
                pc = INSN_CFSD(pc);
            else if (COR)
                pc = INSN_COR(pc);
...
int main() {
    ...
    infile = fopen("test2.out", "rb");
    outfile = fopen("print_2_tx.txt", "w");
    int rc;
    ...
    printf("j_stall:%d\n", jc);
    cy2++;
    printf("cycles: %d,  num_ins: %d\n", cy2, cnt);
    printf("CPI:%lf\n", (1.0 * cy2) / (1.0 * cnt));
    printf("hit times of Data Cache:%d\nmiss times of Data Cache:%d\nMiss rates of Data Cache:%lf\n",d_hit,d_miss ,(1.0*d_miss)/(1.0*(d_miss+d_hit)));
    printf("hit times of Insn Cache:%d\nmiss times of Insn Cache:%d\nMiss rates of Insn Cache:%lf\n",i_hit,i_miss, (1.0*i_miss)/(1.0*(i_miss+i_hit)));
    free(mem);
    fclose(outfile);
    return 0;
}